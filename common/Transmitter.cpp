...
bool Transmitter::getClientId(int server_id, int& client_id) {
	...
	if ( (client_id = accept(server_id, (struct sockaddr*)&client_addr, (socklen_t * __restrict__)&len)) == -1) {
...
void Transmitter::closeSocket(int socket_id) {
	close(socket_id);
}
bool Transmitter::sendData(int socket_id, const std::vector<unsigned char>& data) {
...
bool Transmitter::readData(int socket_id, std::vector<unsigned char>& data, int len) {
	data.resize(len);
	int rec_len = 0;
	while (rec_len < len) {
