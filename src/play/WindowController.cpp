...
WindowController::WindowController(int n_layers, cv::Size top_layer_size,
                                   cv::Size winsize) {
  assert(n_layers > 0);
  ...
  for (int i = n_layers - 1; i >= 0; --i) {
    if (m_layer_size[i].width < winsize.width &&
        m_layer_size[i].height < winsize.height) {
      init_layer = i;
      break;
    }
  }
...
void WindowController::setXYZ(double x, double y, double z) {
  //
  this->x = x;
  this->y = y;
  this->z = z;
