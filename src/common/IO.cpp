...
bool IO::saveMat(std::ofstream &fout, const cv::Mat &mat) { return false; }
bool IO::loadMat(std::ifstream &fin, cv::Mat &mat) { return false; }
...
bool IO::saveStringList(std::ofstream &fout,
...
bool IO::loadStringList(std::ifstream &fin, std::vector<std::string> &list) {
  string str;
  while (fin >> str) {
    list.push_back(str);
  }
