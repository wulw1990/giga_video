...
static void work_listen(int server_id) {
  //
  while (1) {
    cout << endl << "work_listen: wait for a client..." << endl;
    int client_id;
    string client_info;
...
PlayerMaster::PlayerMaster(std::string path, int video_mode, int server_port,
  ...
  m_window_width = 1024;
  m_window_height = 720;
  m_server_id = Transmitter::initSocketServer(server_port);
  m_thread_listen = thread(work_listen, m_server_id);

  m_info.m_waiter = make_shared<WaiterServer>(path, m_window_width,
                                              m_window_height, video_mode);
  m_info.win_title = "giga PlayerMaster";
  m_info.mouse_x = -100;
  m_info.mouse_y = -100;
  m_info.mouse_color = Scalar(255, 255, 255);
  m_info.thumnail_index = -1;
  cout << "PlayerMaster: m_info init ok" << endl;
  ...
  cout << "PlayerMaster: thumnail ok" << endl;
...
void PlayerMaster::drawMouse(cv::Mat &show, int x, int y, Scalar color) {
...
void PlayerMaster::play() {
  ...
  while (1) {
    ...
    // cout << "PlayerMaster time: " << time << " index: " <<
...
int PlayerMaster::getThunbmailIndex(int x, int y,
...
void PlayerMaster::onMouse(int event, int x, int y, int, void *data) {
